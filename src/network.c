    ...
    for (uint8_t i = 1; i <= n_shares; i++) {
        if (i == party_id) {
            ...
            switch (shares_type) {
                case BC_CURVE_POINTS:
#ifdef _WIN32
                    read_status = recv(sock, &shares.points[i - 1],
                                       sizeof(shares.points[i - 1]),0);
#else                
                ...
                case BC_BIGNUM:
#ifdef _WIN32
                    read_status = recv(sock, &shares.nums[i - 1],
                                       sizeof(shares.nums[i - 1]),0);
#else                   
...
#ifdef _WIN32
                    read_status =
                        recv(sock, shares.bytes[i - 1], BC_BYTES_SIZE,0);
#else                   
                    read_status =
...
#ifdef _WIN32
            if (recv(sock, &eval, sizeof(eval),0) < 0)
#else  
